...
# === Step 4: Apply Checkpoint Suppression ===
def apply_checkpoint_modulation(tensor, suppression_mask, reactivation_rate=0.5):
...
# Generate symbolic suppression mask
suppression_mask = np.random.choice([0, 1], size=tensor.shape, p=[0.5, 0.5])
tensor = apply_checkpoint_modulation(tensor, suppression_mask, reactivation_rate=0.5)

tensor = collapse_tensor_multifreq(tensor)

# === Output ===
print("Checkpoint modulation applied.")
